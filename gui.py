...
class HelloFrame(wx.Frame):
    ...
    def __init__(self,*args,**kw):
        ...
        self.pnl = pnl
        st = wx.StaticText(pnl, label="传统节日特征物品识别", pos=(200, 0))
        font = st.GetFont()
        font.PointSize += 10
        font = font.Bold()
        st.SetFont(font)
        ...
        # 选择图像文件按钮
        btn = wx.Button(pnl, -1, "选择图像")
        btn.Bind(wx.EVT_BUTTON, self.OnSelect)

        self.makeMenuBar()

        self.CreateStatusBar()
        self.SetStatusText("欢迎识别")
    ...
    def OnSelect(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            img = Image.open(dialog.GetPaths()[0])
            ...
            self.initimage(name= dialog.GetPaths()[0])
...
if __name__ == '__main__':
    ...
    app = wx.App()
    frm = HelloFrame(None, title='中国传统节日像征识别', size=(1000,600))
    frm.Show()
    app.MainLoop()